# Instagram Clone

## Tech stack

**Languages** :

![TypeScript Badge](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=fff&style=for-the-badge)


**Frontend** :

![Next.js Badge](https://img.shields.io/badge/Next.js-000?logo=nextdotjs&logoColor=fff&style=for-the-badge)
![React Badge](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=000&style=for-the-badge)
![Base UI Badge](https://img.shields.io/badge/Base%20UI-EDEDED?logo=baseui&logoColor=000&style=for-the-badge)


**Backend** :

![GraphQL Badge](https://img.shields.io/badge/GraphQL-E10098?logo=graphql&logoColor=fff&style=for-the-badge)
![Apollo GraphQL Badge](https://img.shields.io/badge/Apollo%20GraphQL-311C87?logo=apollographql&logoColor=fff&style=for-the-badge)


![NestJS Badge](https://img.shields.io/badge/NestJS-E0234E?logo=nestjs&logoColor=fff&style=for-the-badge)
![Fastify Badge](https://img.shields.io/badge/Fastify-000?logo=fastify&logoColor=fff&style=for-the-badge)
![TypeORM Badge](https://img.shields.io/badge/TypeORM-FE0803?logo=typeorm&logoColor=fff&style=for-the-badge)


![PostgreSQL Badge](https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=fff&style=for-the-badge)
![ZincSearch Badge](https://img.shields.io/badge/ZincSearch-5BA37F?logo=zincsearch&logoColor=fff&style=for-the-badge)
![MongoDB Badge](https://img.shields.io/badge/MongoDB-47A248?logo=mongodb&logoColor=fff&style=for-the-badge)
![RustFS Badge](https://img.shields.io/badge/RustFS-0196D0?logo=rustfs&logoColor=fff&style=for-the-badge)
![Apache Kafka Badge](https://img.shields.io/badge/Apache%20Kafka-231F20?logo=apachekafka&logoColor=fff&style=for-the-badge)
![Redis Badge](https://img.shields.io/badge/Redis-FF4438?logo=redis&logoColor=fff&style=for-the-badge)

**Operations** :

![.ENV Badge](https://img.shields.io/badge/.ENV-ECD53F?logo=dotenv&logoColor=000&style=for-the-badge)
![Nx Badge](https://img.shields.io/badge/Nx-143055?logo=nx&logoColor=fff&style=for-the-badge)
![Docker Badge](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=fff&style=for-the-badge)


![OpenTelemetry Badge](https://img.shields.io/badge/OpenTelemetry-000?logo=opentelemetry&logoColor=fff&style=for-the-badge)


![GitHub Actions Badge](https://img.shields.io/badge/GitHub%20Actions-2088FF?logo=githubactions&logoColor=fff&style=for-the-badge)


![Claude Badge](https://img.shields.io/badge/Claude-D97757?logo=claude&logoColor=fff&style=for-the-badge)

## TODOs

- [ ] Configure `Hot Module Replacement (HMR)` for `NestJS`.

- [ ] Shift from using `cookies` to `sessions` for authentication.

## REFERENCEs

### NestJS

- [NestJS Authentication + Refresh Token With Passport.js](https://www.youtube.com/watch?v=S8Cjx5ua2JU)

- [NestJS + TypeORM Tutorial | Repositories, Relations, Migrations & More](https://www.youtube.com/watch?v=9MGKKJTwicM)

- [Nest.js Caching Tutorial in 15 Minutes (Redis + Unit Testing)](https://www.youtube.com/watch?v=KXnkhWRCj40)

- [OpenTelemetry NestJS Implementation Guide: Complete Setup for Production [2026]](https://signoz.io/blog/opentelemetry-nestjs/)

- [Designing Notifications Service for Instagram](https://www.youtube.com/watch?v=kIP8L-CSl2Y)

### GraphQL

- [GraphQL Authentication: JWT, login, signup, and more! | NestJS PassportJS Tutorial](https://www.youtube.com/watch?v=XPSSgAPjTb4)

- [Implementing pagination with GraphQL in NestJS](https://blog.logrocket.com/implementing-pagination-graphql-nestjs/)

- [GraphQL Subscriptions: Why we use SSE/Fetch over Websockets](https://wundergraph.com/blog/deprecate_graphql_subscriptions_over_websockets)

- [Automatic Persisted Queries](https://www.apollographql.com/docs/apollo-server/performance/apq#cache-configuration)

- [Server-Side Caching](https://www.apollographql.com/docs/apollo-server/performance/caching#caching-with-responsecacheplugin-advanced) and [Response Cache Eviction](https://www.apollographql.com/docs/apollo-server/performance/response-cache-eviction)

- [Nest.js GraphQL Integration Testing Tutorial](https://www.youtube.com/watch?v=pm3bLJZlkEw)

- [Build GraphQL Microservices With Nest.js & Apollo Federation 2](https://www.youtube.com/watch?v=svQjC6EXEQ4)

- [Demystifying Cache Normalization](https://www.apollographql.com/blog/demystifying-cache-normalization)

- [More resilient code with data masking in Apollo Client 3.12](https://www.apollographql.com/blog/more-resilient-code-with-data-masking-in-apollo-client-3-12)

### NextJS

- [Composition, Caching, and Architecture in modern Next.js](https://www.youtube.com/watch?v=iRGc8KQDyQ8)

- [STOP Structuring Your Project Like a Junior Dev (Do This Instead!)](https://www.youtube.com/watch?v=WJBIProZAHc)

### Miscellaneous

- [Help: NX with Nest [Webpack -> Rspack]](https://github.com/nrwl/nx/discussions/30443#discussioncomment-14201114)

- [Crash when running tsgolint: Option 'baseUrl' has been removed.](https://github.com/oxc-project/tsgolint/issues/351#issuecomment-3520969492)

- [[feature] nest hot reload ](https://github.com/nrwl/nx/issues/1992)

- [Code-first approach future compatibility with ESM modules](https://github.com/nestjs/graphql/issues/2568#issuecomment-1655658256)
