...
use super::ports::{FollowshipsRepository, FeedsRepository, PostCreatedEvent};
...
impl Usecases {
	pub async fn pushPostToFeeds(&self, postCreatedEvent: PostCreatedEvent) -> Result<( )> {
		let followerIds= self.followshipsRepository.getAllFollowers(postCreatedEvent.ownerId).await?;
		self.feedsRepository.pushPostToFeeds(followerIds, postCreatedEvent.id)
	}
