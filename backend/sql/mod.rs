...
pub mod queries {
  pub mod feeds_microservice {
    ...
    pub fn getAllFollowers() -> GetAllFollowersStmt {
      GetAllFollowersStmt(cornucopia_async::private::Stmt::new(
        "SELECT follower_id FROM followships
	WHERE followee_id= $1",
      ))
    }
    pub struct GetAllFollowersStmt(cornucopia_async::private::Stmt);
    impl GetAllFollowersStmt {
  ...
  pub mod posts_microservice {
    ...
    #[derive(Clone, Copy, Debug)]
    pub struct GetPostsOfUserParams {
      pub owner_id: i32,
      pub pageSize: i64,
      pub offset: i64,
    }
    ...
    pub struct GetPostsOfUser {
      pub id: i32,
      pub description: String,
      pub created_at: time::PrimitiveDateTime,
    }
    pub struct GetPostsOfUserBorrowed<'a> {
      pub id: i32,
      pub description: &'a str,
      pub created_at: time::PrimitiveDateTime,
    }
    impl<'a> From<GetPostsOfUserBorrowed<'a>> for GetPostsOfUser {
      fn from(
        GetPostsOfUserBorrowed {
          id,
          description,
          created_at,
        }: GetPostsOfUserBorrowed<'a>,
      ) -> Self {
        Self {
          id,
    ...
    pub struct GetPostsOfUserQuery<'a, C: GenericClient, T, const N: usize> {
      ...
      params: [&'a (dyn postgres_types::ToSql + Sync); N],
      stmt: &'a mut cornucopia_async::private::Stmt,
      extractor: fn(&tokio_postgres::Row) -> GetPostsOfUserBorrowed,
      ...
      C: GenericClient,
    {
      pub fn map<R>(
        self,
        mapper: fn(GetPostsOfUserBorrowed) -> R,
      ) -> GetPostsOfUserQuery<'a, C, R, N> {
        GetPostsOfUserQuery {
          client: self.client,
          params: self.params,
          stmt: self.stmt,
    ...
    pub struct GetPostsQuery<'a, C: GenericClient, T, const N: usize> {
      ...
      params: [&'a (dyn postgres_types::ToSql + Sync); N],
      stmt: &'a mut cornucopia_async::private::Stmt,
      extractor: fn(&tokio_postgres::Row) -> GetPostsBorrowed,
    ...
    where
      C: GenericClient,
    {
      pub fn map<R>(self, mapper: fn(GetPostsBorrowed) -> R) -> GetPostsQuery<'a, C, R, N> {
        GetPostsQuery {
          client: self.client,
          params: self.params,
          stmt: self.stmt,
  ...
  VALUES ($1, $2)
    ...
    impl<'a, C: GenericClient, T1: cornucopia_async::StringSql>
      cornucopia_async::Params<'a, CreateParams<T1>, I32Query<'a, C, i32, 2>, C> for CreateStmt
    {
      fn params(
        &'a mut self,
        client: &'a C,
    ...
    pub fn getPostsOfUser() -> GetPostsOfUserStmt {
      GetPostsOfUserStmt(cornucopia_async::private::Stmt::new(
        "SELECT id, description, created_at FROM posts
	WHERE owner_id= $1
	LIMIT $2 OFFSET $3",
      ))
    }
    pub struct GetPostsOfUserStmt(cornucopia_async::private::Stmt);
    impl GetPostsOfUserStmt {
      pub fn bind<'a, C: GenericClient>(
        &'a mut self,
        client: &'a C,
        owner_id: &'a i32,
        pageSize: &'a i64,
        offset: &'a i64,
      ) -> GetPostsOfUserQuery<'a, C, GetPostsOfUser, 3> {
        GetPostsOfUserQuery {
          client,
          params: [owner_id, pageSize, offset],
          stmt: &mut self.0,
          extractor: |row| GetPostsOfUserBorrowed {
            id: row.get(0),
            description: row.get(1),
            created_at: row.get(2),
          },
          mapper: |it| <GetPostsOfUser>::from(it),
        }
      }
    }
    impl<'a, C: GenericClient>
      cornucopia_async::Params<
        'a,
        GetPostsOfUserParams,
        GetPostsOfUserQuery<'a, C, GetPostsOfUser, 3>,
        C,
      > for GetPostsOfUserStmt
    {
      fn params(
        &'a mut self,
        client: &'a C,
        params: &'a GetPostsOfUserParams,
      ) -> GetPostsOfUserQuery<'a, C, GetPostsOfUser, 3> {
        self.bind(client, &params.owner_id, &params.pageSize, &params.offset)
      }
    }
    pub fn getPosts() -> GetPostsStmt {
      GetPostsStmt(cornucopia_async::private::Stmt::new(
        "SELECT id, owner_id, description, created_at FROM posts
	WHERE id= ANY($1)",
      ))
    }
    pub struct GetPostsStmt(cornucopia_async::private::Stmt);
    impl GetPostsStmt {
      pub fn bind<'a, C: GenericClient, T1: cornucopia_async::ArraySql<Item = i32>>(
        &'a mut self,
        client: &'a C,
        post_ids: &'a T1,
      ) -> GetPostsQuery<'a, C, GetPosts, 1> {
        GetPostsQuery {
          client,
          params: [post_ids],
          stmt: &mut self.0,
          extractor: |row| GetPostsBorrowed {
            id: row.get(0),
            owner_id: row.get(1),
