...
import { isDevelopmentEnvironment } from "./modules/root/module"
...
async function main() {
  ...
  app.getHttpAdapter().use(
    "/graphql/stream",
    createHandler({
      schema: app.get(GraphQLSchemaHost)
    })
  )
  ...
  */
  ...
  /*
    ...
    This can combine an HTTP server with a microservice listener or even just multiple different
    microservice listeners.

    NOTE : By default a hybrid application will not inherit global pipes, interceptors, guards and
           filters configured for the main (HTTP-based) application.
  */
  app.connectMicroservice<MicroserviceOptions>({
    transport: Transport.KAFKA,
    options: {
      client: {
        brokers: configService.getOrThrow("REDPANDA_BROKERS").split(","),
        ssl: false,
        sasl: isDevelopmentEnvironment
          ? undefined
          : {
              mechanism: KAFKA_SASL_MECHANISM,
              username: configService.getOrThrow("REDPANDA_USERNAME"),
              password: configService.getOrThrow("REDPANDA_PASSWORD")
            }
      },
