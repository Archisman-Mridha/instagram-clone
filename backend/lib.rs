...
pub mod utils {
  ...
  pub fn createPgConnectionPool( ) -> Pool {
	...
	pub mod observability {
		...
		use tokio::spawn;
		use tracing::{Span, warn, subscriber::set_global_default, debug, level_filters::LevelFilter, debug_span};
		use tracing_opentelemetry::{OpenTelemetryLayer, OpenTelemetrySpanExt};
		use warp::Filter;
		use super::getEnv;
		use tracing_subscriber::{Registry, layer::SubscriberExt, EnvFilter, Layer};
		...
		pub fn makeSpan(request: &Request<Body>) -> Span {
			...
			debug_span!("Incoming Request", ?headers, trace_id = tracing::field::Empty)
		}
				...
				impl<'de> serde::de::Visitor<'de> for DbOperationVisitor {
					...
					fn visit_str<E>(self, value: &str) -> Result<DbOperation, E>
						...
						match value {
							...
							_ => Err(E::custom(format!("ERROR : Invalid value for 'op': {}", value))),
						}
					}
				}
